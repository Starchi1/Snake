...
def gameLoop():  # Основная игровая функция которая наследует все остальные функции
    global lastMove
    global running
    global imeg
    x1 = dis_width / 2
    ...
    # Проверяет чтобы координаты не повторялись
    while prov2:
        if foodx1 != foodx2 and foodx1 != foodx3 and foodx1 != foodx4 and foodx1 != foodx5 and foodx1 != foodx6:
            prov2 = False
        elif foody1 != foody2 and foody1 != foody3 and foody1 != foody4 and foody1 != foody5 and foody1 != foody6:
            prov2 = False
        else:
            foodx1 = round(randrange(0, dis_width - snake_block) / 10.0) * 10.0
            foody1 = round(randrange(50, dis_height - snake_block) / 10.0) * 10.0
