...
def number_dr(x, y, n):  # Написание числа на кубике
    value = number.render(str(n), True, 'red')
    screen.blit(value, [x, y-2])


def Your_score(score):  # Рисует счет игрока
    value = score_font.render("Your Score: " + str(score), True, 'blue')
    screen.blit(value, [0, 0])


def our_snake(snake_block, snake_list):  # Создание змейки
    for x in snake_list:
        pygame.draw.rect(screen, 'black', [x[0], x[1], snake_block, snake_block])


def message():  # Отображение финального окна
...
def game():  # Основная игровая функция которая наследует все остальные функции
    global lastMove
    global running
    global imeg
    x1 = dis_width / 2
    ...
    prov = True
    prov2 = True
    #  Проверяет, чтобы не числа не повторялись
    while prov:
        if n4 != n5 and n4 != n6 or n5 != n4 and n5 != n6:
            if n3 != n4 or n3 != n5 or n3 != n6:
                if n1 != n4 or n1 != n5 or n1 != n6:
                    if n2 != n4 or n2 != n5 or n2 != n6:
                        prov = False
        else:
            n4 = randrange(1, 50, 5)
            n5 = randrange(51, 99, 5)
    ...
    while running:
        ...
        # Отображение бонусов
        pygame.draw.rect(screen, 'white', [foodx1, foody1, snake_block, snake_block])
        pygame.draw.rect(screen, 'white', [foodx2, foody2, snake_block, snake_block])
        ...
        # Проверка того, что змейка заходит на бонус
